...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        const camera = new THREE.PerspectiveCamera( 40, window.innerWidth / window.innerHeight, 10, 30 );
        ...
        //loader.load( "andreoidBlender.glb", function ( gltf ) {
         ...
         vidro = model.children[1].children[0].children[2];
         rosto = model.children[1].children[0].children[0];
         olhoDireito = model.children[1].children[0].children[0].children[1];
         olhoEsquerdo = model.children[1].children[0].children[0].children[2];
         boca = model.children[1].children[0].children[0].children[0];
         corpo = model.children[1];
         cabeça = model.children[1].children[0];
         braçoDireito = model.children[1].children[3];
         braçoEsquerdo = model.children[1].children[4];
         maoDireita = model.children[1].children[3].children[0];
         maoEsquerda = model.children[1].children[4].children[0];
         baseAntena = model.children[1].children[0].children[1];
         baseCaboAntena = model.children[1].children[0].children[1].children[0];
         caboAntena = model.children[1].children[0].children[1].children[0].children[0];
         esferaAntena = model.children[1].children[0].children[1].children[0].children[0].children[0];


         model.children[0].add(shadowMesh);
...
const folder1 = panel.addFolder( 'cameraLookAt  XYZ _|\:' );
const folder2 = panel.addFolder( 'cameraParams' );
const folder3 = panel.addFolder( 'cameraPos' );
const folder4 = panel.addFolder( 'background' );
const folder5 = panel.addFolder( 'crop' );
const folder6 = panel.addFolder( 'grid' );
//const camera = new THREE.PerspectiveCamera( 40, window.innerWidth / window.innerHeight, 10, 30 );
folder1.open();
folder2.open();
folder3.open();
...
var panelSettings = {
  ...
  'currentWidth': renderer.getSize(new THREE.Vector2()).x,
  'currentHeight': renderer.getSize(new THREE.Vector2()).y,
  'desiredWidth' : 1,
  'desiredHeight' : 1,
  'xCropOffset': 0,
  'yCropOffset': 0,
...
folder1.add(panelSettings, 'cameraLookAt').onChange(function () {
  if( panelSettings.cameraLookAt == false){ camera.updateProjectionMatrix(); }
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
  
});

folder1.add( panelSettings, 'x', -20.0, 20.0, 0.01 ).onChange(function () {
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder1.add( panelSettings, 'y', -20.0, 20.0, 0.01 ).onChange(function () {
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder1.add( panelSettings, 'z', -20.0, 20.0, 0.01 ).onChange(function () {
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); } 
});


folder2.add( camera, 'fov', 10, 300, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder2.add( camera, 'zoom', 0.01, 100, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder2.add( camera, 'near', 0.01, 100, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder2.add( camera, 'far', 0.01, 100, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});

folder3.add( camera.position, 'x', -100, 100, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder3.add( camera.position, 'y', -100, 100, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
folder3.add( camera.position, 'z', -100, 100, 0.01).listen().onChange(function () {
  camera.updateProjectionMatrix()
  if( panelSettings.cameraLookAt == true) { camera.lookAt(panelSettings.x,panelSettings.y,panelSettings.z); }
});
...
folder4.add(panelSettings, 'backGroundVis').onChange(function () {
  if( panelSettings.backGroundVis == false){ scene.background = null; } else { scene.background = new THREE.Color( 0x0296a6 ); }
  
});

folder5.add( panelSettings, 'currentWidth', 0, 1920, 0.01).onChange(function () {
  panelSettings.currentWidth = renderer.getSize(new THREE.Vector2()).x;
});

folder5.add( panelSettings, 'currentHeight', 0.001, 1080, 0.01).onChange(function () {
  panelSettings.currentHeight = renderer.getSize(new THREE.Vector2()).y;
});

folder5.add( panelSettings, 'desiredWidth', 0.001, 2, 0.001).onChange(function () {
  const x = (panelSettings.currentWidth) * (panelSettings.desiredWidth);
  const y = (panelSettings.currentHeight) * (panelSettings.desiredHeight);
  const xOff = (panelSettings.currentWidth) * (panelSettings.xCropOffset);
  const yOff = (panelSettings.currentHeight) * (panelSettings.yCropOffset);

  renderer.setScissor( xOff, yOff, x, y);
  renderer.setScissorTest(true);
});

folder5.add( panelSettings, 'desiredHeight', 0.001, 2, 0.001).onChange(function () {
  const x = (panelSettings.currentWidth) * (panelSettings.desiredWidth);
  const y = (panelSettings.currentHeight) * (panelSettings.desiredHeight);
  const xOff = (panelSettings.currentWidth) * (panelSettings.xCropOffset);
  const yOff = (panelSettings.currentHeight) * (panelSettings.yCropOffset);
...
folder5.add( panelSettings, 'xCropOffset', 0, 2, 0.001).onChange(function () {
  const x = (panelSettings.currentWidth) * (panelSettings.desiredWidth);
  const y = (panelSettings.currentHeight) * (panelSettings.desiredHeight);
  const xOff = (panelSettings.currentWidth) * (panelSettings.xCropOffset);
  const yOff = (panelSettings.currentHeight) * (panelSettings.yCropOffset);
...
folder5.add( panelSettings, 'yCropOffset', 0, 2, 0.001).onChange(function () {
  const x = (panelSettings.currentWidth) * (panelSettings.desiredWidth);
  const y = (panelSettings.currentHeight) * (panelSettings.desiredHeight);
  const xOff = (panelSettings.currentWidth) * (panelSettings.xCropOffset);
  const yOff = (panelSettings.currentHeight) * (panelSettings.yCropOffset);
...
folder6.add(panelSettings, 'grid').onChange(function () {
  if( panelSettings.grid == false){ gridHelper.visible = false; } else { gridHelper.visible = true; }
  
});
//----------------------------------------------------------APAGAR ACIMA QUANDO O ANGULO E A POSICAO DA IMAGEM ESTIFEREM DEFINIDAS-----------------  
...
function animate() {
  ...
  if (mixer !== null) {
    ...
    shadowMesh.material.opacity = THREE.MathUtils.lerp(.9, .05, model.children[1].position.y);
